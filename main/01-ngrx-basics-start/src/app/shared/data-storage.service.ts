...
export class DataStorageService {
  private authDomain = `https://nomad-recipe-book.firebaseio.com`;
  ...
  storeRecipes() {
    ...
    //   // headers: headers
    // });
    const req = new HttpRequest('PUT', `${this.authDomain}/recipes.json`, this.recipeService.getRecipes(), {reportProgress: true});
    return this.httpClient.request(req);
  }

  getRecipes() {
    // this.httpClient.get<Recipe[]>('https://ng-recipe-book-3adbb.firebaseio.com/recipes.json?auth=' + token)
    this.httpClient.get<Recipe[]>(`${this.authDomain}/recipes.json`, {
      observe: 'body',
      responseType: 'json'
    })
      .map(
